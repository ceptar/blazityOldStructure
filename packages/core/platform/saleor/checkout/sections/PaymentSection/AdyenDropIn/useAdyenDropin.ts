...
import { useEvent } from "@enterprise-commerce/core/platform/saleor/checkout/hooks/useEvent";
import { useSubmit } from "@enterprise-commerce/core/platform/saleor/checkout/hooks/useSubmit";
import {
...
} from "@enterprise-commerce/core/platform/saleor/checkout/sections/PaymentSection/AdyenDropIn/types";
import {
  anyFormsValidating,
  areAllFormsValid,
  useCheckoutValidationActions,
  useCheckoutValidationState,
} from "@enterprise-commerce/core/platform/saleor/checkout/state/checkoutValidationStateStore";
import {
  clearQueryParams,
  getQueryParams,
...
import { type ParsedAdyenGateway } from "@enterprise-commerce/core/platform/saleor/checkout/sections/PaymentSection/types";
import {
  areAnyRequestsInProgress,
  hasFinishedApiChangesWithNoError,
  useCheckoutUpdateState,
  useCheckoutUpdateStateActions,
} from "@enterprise-commerce/core/platform/saleor/checkout/state/updateStateStore";
import { useCheckoutComplete } from "@enterprise-commerce/core/platform/saleor/checkout/hooks/useCheckoutComplete";
import { useErrorMessages } from "@enterprise-commerce/core/platform/saleor/checkout/hooks/useErrorMessages";
import { adyenErrorMessages } from "@enterprise-commerce/core/platform/saleor/checkout/sections/PaymentSection/AdyenDropIn/errorMessages";
...
import { getUrlForTransactionInitialize } from "@enterprise-commerce/core/platform/saleor/checkout/sections/PaymentSection/utils";
import { usePaymentProcessingScreen } from "@enterprise-commerce/core/platform/saleor/checkout/sections/PaymentSection/PaymentProcessingScreen";
...
export const useAdyenDropin = (props: AdyenDropinProps) => {
  ...
  const { errorMessages: commonErrorMessages } =
    useErrorMessages(apiErrorMessages);
    ...
    useMemo(
      () => ({
        ...
        extractCustomErrors: (result) =>
          result?.data?.transactionInitialize?.data?.errors,
  ...
  >(
    useMemo(
      () => ({
        ...
        extractCustomErrors: (result) =>
          result?.data?.transactionProcess?.data?.errors,
        onSuccess: ({ data }) => {
          if (!data?.data) {
            showCustomErrors([
              { message: commonErrorMessages.somethingWentWrong },
  ...
  const onSubmitInitialize: AdyenCheckoutInstanceOnSubmit = useEvent(
  ...
  const onAdditionalDetails: AdyenCheckoutInstanceOnAdditionalDetails =
    useEvent(async (state, component) => {
      setAdyenCheckoutSubmitParams({ state, component });
      if (currentTransactionId) {
        adyenCheckoutSubmitParams?.component?.setStatus("loading");
        setSubmitInProgress(true);
