...
export const useBillingSameAsShippingForm = (
  { autoSave, onSetBillingSameAsShipping }: BillingSameAsShippingFormProps = {
    autoSave: false,
  }
) => {
  ...
  >({
    ...
    shouldAbort: () =>
      !formBillingAddress || !Object.keys(formBillingAddress).length,
    onStart: () => {
  ...
  const previousBillingSameAsShipping = useRef(
    initialValues.billingSameAsShipping
  );
  ...
  useEffect(() => {
    if (
      formBillingAddress &&
      !isMatchingAddress(billingAddress, formBillingAddress)
    ) {
      handleSubmit();
    }
  }, [
