...
import { type CheckoutScope } from "@enterprise-commerce/core/platform/saleor/checkout/hooks/useAlerts";

export type CheckoutUpdateStateStatus = "success" | "loading" | "error";

export type CheckoutUpdateStateScope = Exclude<
  CheckoutScope,
  "checkoutPay" | "checkoutFinalize"
>;
...
const useCheckoutUpdateStateStore =
  createWithEqualityFn<CheckoutUpdateStateStore>(
    (set) => ({
      ...
      actions: {
        setSubmitInProgress: (submitInProgress: boolean) =>
          set({ submitInProgress }),
        setShouldRegisterUser: (shouldRegisterUser: boolean) =>
          set({
            shouldRegisterUser,
          }),
        setLoadingCheckout: (loading: boolean) =>
          set({ loadingCheckout: loading }),
        setChangingBillingCountry: (changingBillingCountry: boolean) =>
          set({ changingBillingCountry }),
...
export const useUserRegisterState = () => {
  const shouldUserRegister = useCheckoutUpdateStateStore(
    (state) => state.shouldRegisterUser
  );
