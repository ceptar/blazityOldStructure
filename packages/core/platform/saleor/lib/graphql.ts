...
export async function executeGraphQL<Result, Variables>(
  ...
  const response = withAuth ? await getServerAuthClient().fetchWithAuth(process.env.NEXT_PUBLIC_SALEOR_API_URL, input) : await fetch(process.env.NEXT_PUBLIC_SALEOR_API_URL, input)
