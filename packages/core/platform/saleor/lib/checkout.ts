...
import { CheckoutCreateDocument, CheckoutFindDocument } from "@enterprise-commerce/core/platform/saleor/codegen/graphqlommerce/core/platform/saleor/codegen/graphql"
...
export function saveIdToCookie(channel: string, checkoutId: string) {
  const shouldUseHttps = process.env.NEXT_PUBLIC_STOREFRONT_URL?.startsWith("https") || !!process.env.NEXT_PUBLIC_VERCEL_URL
  const cookieName = `checkoutId-${channel}`
...
export const create = ({ channel }: { channel: string }) => executeGraphQL(CheckoutCreateDocument, { cache: "no-cache", variables: { channel } })
