...
import { type UserDetailsFragment } from "@enterprise-commerce/core/platform/saleor/codegen/graphql"
import { logout } from "@enterprise-commerce/core/platform/saleor/actions"
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right divide-y divide-neutral-200 bg-white py-1 text-start shadow ring-1 ring-neutral-200 ring-opacity-5 focus:outline-none">
          ...
          <div className="flex flex-col px-1 py-1">
            <Menu.Item>
              {({ active }) => (
                <LinkWithChannel href="/orders" className={clsx(active && "bg-neutral-100", "block px-4 py-2 text-sm font-medium text-neutral-500 hover:text-neutral-700")}>
                  My orders
                </LinkWithChannel>
              )}
            </Menu.Item>
          ...
          <div className="flex flex-col px-1 py-1">
            <Menu.Item>
              {({ active }) => (
                <form action={logout}>
                  <button type="submit" className={clsx(active && "bg-neutral-100", "block px-4 py-2 text-start text-sm font-medium text-neutral-500 hover:text-neutral-700")}>
                    Log Out
                  </button>
                </form>
              )}
